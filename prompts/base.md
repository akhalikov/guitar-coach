...
Because logs (`journal/`) and any system edits (prompts, curriculum) are in the same repo, a session produces **one commit**, not two. `git add -A` from the repo root stages everything.

### Heatmap — built by CI, not locally

`journal/practice-heatmap.html` is a contribution graph of practice days. It's generated by `journal/tools/gen_heatmap.py` (reads `journal/logs/` + `journal/lessons.txt`), which also emits a static `journal/practice-heatmap.svg` snapshot for embedding in markdown (the README shows it inline).

> **Setup requirement:** the live site only reflects the CI-built heatmap if the repo's **Pages Source is set to "GitHub Actions"** (Settings → Pages). If it's on "Deploy from a branch", GitHub serves the *committed* `practice-heatmap.html` (which lags) and the workflow's fresh build is ignored. **This regen now runs automatically in GitHub Actions on every push** (`.github/workflows/pages.yml`) — so the coach does **NOT** add a `gen_heatmap.py` step to the commit line. Push the log; CI rebuilds the heatmap and redeploys the live dashboard within ~1 minute.

The committed copy of `practice-heatmap.html` is just a seed and may lag; the **live site is canonical**. Only run `gen_heatmap.py` by hand if you specifically want a fresh local copy to open by double-click.
...
Same for a session commit or a system edit — the heatmap rebuilds in CI, so there's no local generate step:

````
```bash
cd ~/github/guitar-coach && rm -f .git/index.lock && git add -A && git commit -m "Practice log 2026-05-23 session 1 (classical)" && git push
```
````
