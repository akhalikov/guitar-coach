...
2. Read `../../journal/progress/electric.md` — **this is where the shared JustinGuitar module and Stine theory position live.** Acoustic does not keep its own copy of that checklist.
3. Read `../../journal/progress/acoustic.md` for acoustic-only state: repertoire lane positions, unplugged/technique notes, open struggles.
4. Read the **3 most recent** daily logs in `../../journal/logs/acoustic/YYYY/` (current year).
...
If `../../journal/logs/acoustic/` has no entries, use the **First Session Flow** below.
...
- **JustinGuitar Grades 1–3** — the practice spine. Outline: `../../curriculum/electric/justinguitar-grades.md`. Current position: `../../journal/progress/electric.md`.
...
- **Daily log stream.** `../../journal/logs/acoustic/YYYY/` — separate from electric's, since these are separate sessions.
...
## First Session Flow (if `../../journal/logs/acoustic/` is empty)
...
6. End with the first acoustic daily log and create `../../journal/progress/acoustic.md` if it doesn't exist yet (template below).
...
| `../../journal/progress/electric.md` | **Shared** JG module + Stine topic — read first, don't duplicate |
| `../../journal/progress/acoustic.md` | Acoustic-only state — lane positions, technique notes, open struggles |
...
| `../../journal/logs/acoustic/YYYY/MM-DD.md` | Daily session journals (journal) |
| `../../journal/logs/acoustic/YYYY/MM-DD-week.md` | Weekly reviews, once set up (journal) |
