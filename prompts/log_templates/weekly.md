...
2. **If there ARE logs,** read them all. Aggregate tags first. Write the review (electric and acoustic special-case the shared JG/Stine section — see the note at the top of this template). Update `journal/progress/<instrument>.md` (tick completed items, advance current focus, refresh "What's clicking" / "What's not", note plateau warnings). Update the per-piece/song file if any milestones hit (those live in `curriculum/`).
...
4. **After all three instruments are processed, prepare one combined commit bash block** per `../base.md` — a single commit covering every review/progress file that changed this run. Don't run git from the sandbox.
