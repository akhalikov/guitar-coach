...
└── guitar-coach-private/       (sibling — private, personal data)
```

Throughout this repo, paths written as `../guitar-coach-private/logs/...` and `../guitar-coach-private/progress/...` refer to that sibling repo. When reading or writing those files, use the absolute path `~/github/guitar-coach-private/` — never create logs or progress files inside this repo.
...
**Student profile** (name, equipment, started dates, practice targets): `../guitar-coach-private/student-profile.md`
...
3. `../guitar-coach-private/progress/electric.md` — current state (current JG module, current Stine topic, songs in rotation, open struggles)
4. The **3 most recent** daily logs in `../guitar-coach-private/logs/electric/<current-year>/` (skip if folder is empty — that's the first-session flow)
...
3. `../guitar-coach-private/progress/electric.md` — **read for current JG module and current Stine topic** (the shared spine — do not re-derive or duplicate these)
4. `../guitar-coach-private/progress/acoustic.md` — acoustic-only state (repertoire lane positions, unplugged/technique notes, open struggles)
5. The **3 most recent** daily logs in `../guitar-coach-private/logs/acoustic/<current-year>/` (skip if folder is empty — that's the first-session flow)
...
5. `../guitar-coach-private/progress/classical.md` — current state summary + rolling notes
6. The **3 most recent** daily logs in `../guitar-coach-private/logs/classical/<current-year>/` (skip if folder is empty)
...
5. Write the daily log with the required tags — to `../guitar-coach-private/logs/<instrument>/<year>/`
6. Update `../guitar-coach-private/progress/<instrument>.md`
...
- ❌ Creating logs or progress files inside this (public) repo. They go in the **sibling private repo** `../guitar-coach-private/`. Anything that's personal data leaves this repo clean.
- ❌ Creating files outside the established structure. Output goes into `../guitar-coach-private/logs/<instrument>/<year>/` or, for milestones, into the per-piece/song detail files in `curriculum/`.
